...
function generatePDFReport(inventory, events, notes, history) {
  ...
  const totalSoldAllEvents = events.reduce((s, e) => s + (e.totalSold || 0), 0);
  const totalRevenueAllEvents = events.reduce((s, e) => s + eventRevenue(e), 0);
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  html += `<div class="page">
    ...
    <div class="summary-grid">
      <div class="summary-card"><div class="label">Total Boxes Sold</div><div class="value">${totalSoldAllEvents}</div></div>
      <div class="summary-card"><div class="label">Total Revenue</div><div class="value">${fmt(totalRevenueAllEvents)}</div></div>
    ...
    <table>
      <tr><th>Cookie</th><th>Boxes</th><th>Cases</th><th>Loose</th><th>$/Box</th><th>Value</th></tr>
      ${inventory.map(c => {
        const loose = c.units % UNITS_PER_CASE;
        return `<tr><td><strong>${c.name}</strong></td><td>${c.units}</td><td>${c.cases}</td><td>${loose}</td><td>${fmt(price(c.name))}</td><td><strong>${fmt(c.units * price(c.name))}</strong></td></tr>`;
      }).join("")}
    ...
    <div class="footer">Cookie HQ Report • Page 1</div>
  </div>`;

  // PAGE 2+: Events
  html += `<div class="page"><h1>🎪 Event Reports</h1><div class="subtitle">All events with sales breakdown and transaction details</div>`;
  if (events.length === 0) {
    html += `<p style="color:#9ca3af;text-align:center;padding:40px">No events recorded yet.</p>`;
  }
  events.forEach((event, idx) => {
    ...
    html += `<div class="event-box">
      <div class="event-header">
        <div><div class="event-name">${event.name}</div><div class="event-meta">📅 ${event.date}${event.location ? ` · 📍 ${event.location}` : ""}</div></div>
      </div>
      <div class="event-stats">
        <div class="event-stat">Boxes Sold: <span style="color:#047857">${event.totalSold || 0}</span></div>
        <div class="event-stat">Revenue: <span style="color:#047857">${fmt(rev)}</span></div>
      </div>`;
    if (soldEntries.length > 0) {
      html += `<table><tr><th>Cookie</th><th>Qty Sold</th><th>Revenue</th></tr>`;
      soldEntries.forEach(([cid, qty]) => {
        const c = inventory.find(i => i.id === Number(cid));
        const r = qty * price(c?.name);
        html += `<tr><td>${c?.name || "Unknown"}</td><td>${qty}</td><td>${fmt(r)}</td></tr>`;
      });
    ...
    if (event.transactions && event.transactions.length > 0) {
      ...
      event.transactions.forEach(tx => {
        html += `<div class="log-row"><div class="log-detail">${tx.cookieName} × ${tx.qty} = ${fmt(tx.qty * price(tx.cookieName))}</div><div class="log-time">${fmtDate(tx.timestamp)}</div></div>`;
      });
    }
    html += `</div>`;
  });
  ...
  // PAGE 3: Notes
  html += `<div class="page"><h1>📝 Notes & IOUs</h1><div class="subtitle">All recorded notes, IOUs, and discrepancies</div>`;
  if (notes.length === 0) {
    html += `<p style="color:#9ca3af;text-align:center;padding:40px">No notes recorded.</p>`;
  }
  notes.forEach(note => {
    html += `<div class="note-card"><div class="note-date">📅 ${fmtDate(note.timestamp)}</div><div class="note-text">${note.text.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</div></div>`;
  });
  html += `<div class="footer">Cookie HQ Report • Notes</div></div>`;

  // PAGE 4: Full Audit Log
  html += `<div class="page"><h1>📋 Full Audit Log</h1><div class="subtitle">Complete changelog of all inventory changes</div>`;
  if (history.length === 0) {
    html += `<p style="color:#9ca3af;text-align:center;padding:40px">No activity recorded.</p>`;
  } else {
  ...
  html += `<div class="footer">Cookie HQ Report • Audit Log</div></div>`;
...
export default function CookieInventoryApp() {
  ...
  // Reset: 3-step validation
  const [resetStep, setResetStep] = useState(0); // 0=hidden, 1=first confirm, 2=second confirm, 3=require note
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const result = await window.storage.get("cookie-hq-data", true);
        ...
        // First time with shared — try to migrate from personal storage
        try {
          const personal = await window.storage.get("cookie-hq-data");
          if (personal && personal.value) {
            const data = JSON.parse(personal.value);
            if (data.inventory) setInventory(data.inventory);
            if (data.events) setEvents(data.events);
            if (data.history) setHistory(data.history);
            if (data.notes) setNotes(data.notes);
            // Save to shared immediately so it persists
            await window.storage.set("cookie-hq-data", personal.value, true);
          }
        } catch (e2) {}
      }
      setLoaded(true);
  ...
  useEffect(() => {
    ...
    const saveData = async () => {
      try {
        await window.storage.set("cookie-hq-data", JSON.stringify({ inventory, events, history, notes }), true);
  ...
  const executeReset = async () => {
    ...
    // Log the reset in history before clearing
    ...
    try { await window.storage.set("cookie-hq-data", JSON.stringify({ inventory: INITIAL_INVENTORY, events: [], history: [resetEntry], notes: [] }), true); } catch (e) {}
  ...
  const exportReport = () => {
    ...
    try {
      ...
      showToast("Report downloaded! Open it and use Print → Save as PDF");
        ...
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.6)", zIndex: 998, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: "white", borderRadius: 16, padding: 24, maxWidth: 380, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
            ...
            {resetStep === 1 && (<>
              ...
              <p style={{ color: "#6b7280", fontSize: 14, textAlign: "center", margin: "0 0 20px", lineHeight: 1.5 }}>This will permanently erase <strong>ALL</strong> events, sales data, notes, and inventory changes. This action <strong>cannot be undone</strong>.</p>
            ...
            {resetStep === 3 && (<>
              ...
              <p style={{ color: "#6b7280", fontSize: 13, textAlign: "center", margin: "0 0 12px" }}>Explain why you're resetting. This will be logged for audit purposes.</p>
              <textarea value={resetNote} onChange={(e) => setResetNote(e.target.value)} placeholder="e.g., End of cookie season, starting fresh for next year..."
                rows={3} style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1px solid #d1d5db", fontFamily: fonts, fontSize: 14, fontWeight: 600, resize: "vertical", boxSizing: "border-box", marginBottom: 12 }} />
              <div style={{ display: "flex", gap: 8 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {inventory.map((cookie) => {
                ...
                const p = price(cookie.name);
                const val = cookie.units * p;
                return (
        ...
        {activeTab === "notes" && (
          <div>
            ...
            <div style={{ background: "white", borderRadius: 14, padding: 14, marginBottom: 16, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", border: "1px solid #f3f4f6" }}>
              <textarea value={newNote} onChange={(e) => setNewNote(e.target.value)} placeholder="Add a note... (IOUs, discrepancies, shared inventory notes, reminders)" rows={3}
              ...
              <div key={h.id} style={{ background: "white", borderRadius: 12, padding: "12px 14px", marginBottom: 8, boxShadow: "0 1px 3px rgba(0,0,0,0.04)", display: "flex", alignItems: "center", gap: 12, border: "1px solid #f3f4f6" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: "#9ca3af", fontWeight: 600 }}>
                    {h.eventName && `at ${h.eventName} · `}{fmtDateShort(h.timestamp)}
                  </div>
                </div>
