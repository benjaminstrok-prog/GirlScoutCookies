...
export default function CookieInventoryApp() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const result = await window.storage.get("cookie-hq-data");
  ...
  useEffect(() => {
    ...
    const saveData = async () => {
      try {
        await window.storage.set("cookie-hq-data", JSON.stringify({ inventory, events, history, notes }));
  ...
  const resetAllData = async () => {
    setInventory(INITIAL_INVENTORY); setEvents([]); setHistory([]); setNotes([]);
    setShowReset(false);
    try { await window.storage.delete("cookie-hq-data"); } catch (e) {}
  ...
  const sellCookies = (eventId, cookieId, qty) => {
    ...
    if (!cookie || cookie.units < qty) {
      showToast(`Not enough ${cookie?.name || "cookies"} in stock!`, "error"); return;
    }
  ...
  const undoSale = (eventId, txId) => {
    ...
    setEvents((prev) => prev.map((e) => {
      ...
      if (newSalesQty <= 0) delete newSales[tx.cookieId]; else newSales[tx.cookieId] = newSalesQty;
  ...
  const adjustInventory = (cookieId) => {
    ...
    const cookie = inventory.find((c) => c.id === cookieId);
    if (!cookie) return;
    if (adjustType === "remove" && cookie.units < qty) { showToast("Not enough units to remove!", "error"); return; }
    setInventory((prev) => prev.map((c) => {
      if (c.id !== cookieId) return c;
  ...
  const eventRevenue = (event) => event.transactions.reduce((sum, tx) => sum + tx.qty * price(tx.cookieName), 0);
  const eventCookieRevenue = (event, cookieId) => {
    const cookieName = inventory.find((c) => c.id === cookieId)?.name;
    return (event.sales[cookieId] || 0) * price(cookieName);
  };
    ...
    <div style={{ fontFamily: fonts, minHeight: "100vh", background: "linear-gradient(135deg, #fef9f0 0%, #f0fdf4 50%, #fefce8 100%)", color: "#1f2937", position: "relative" }}>
      ...
      {showReset && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", zIndex: 998, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: "white", borderRadius: 16, padding: 24, maxWidth: 340, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            ...
            <p style={{ color: "#6b7280", fontSize: 14, textAlign: "center", margin: "0 0 20px" }}>This will erase all events, sales, notes, and reset inventory. Cannot be undone.</p>
      ...
      <div style={{ padding: "16px 16px 100px" }}>
        ...
        {activeTab === "events" && (
          <div>
            {!showAddEvent ? (
              <button onClick={() => setShowAddEvent(true)} style={{ width: "100%", padding: 16, borderRadius: 14, border: "2px dashed #a7f3d0", background: "#ecfdf5", fontFamily: fonts, fontWeight: 800, fontSize: 15, cursor: "pointer", color: "#047857", marginBottom: 16 }}>
                ＋ Add New Event
              </button>
            ) : (
                  ...
                  {isActive && (
                    <div style={{ padding: "0 16px 16px", borderTop: "1px solid #f3f4f6" }}>
                      ...
                      <div style={{ margin: "12px 0", padding: 12, background: "linear-gradient(135deg, #ecfdf5, #f0fdf4)", borderRadius: 10, border: "1px solid #a7f3d0", display: "flex", justifyContent: "space-around", textAlign: "center" }}>
                        <div>
                          <div style={{ fontSize: 11, fontWeight: 700, color: "#6b7280" }}>BOXES SOLD</div>
                          <div style={{ fontSize: 22, fontWeight: 900, color: "#1f2937" }}>{event.totalSold}</div>
                        </div>
                        <div style={{ width: 1, background: "#a7f3d0" }} />
                        <div>
                          <div style={{ fontSize: 11, fontWeight: 700, color: "#6b7280" }}>REVENUE</div>
                          <div style={{ fontSize: 22, fontWeight: 900, color: "#047857" }}>{fmt(revenue)}</div>
                        </div>
                      </div>
                      ...
                      {soldEntries.length > 0 && (
                        <div style={{ margin: "0 0 12px", padding: 10, background: "#f0fdf4", borderRadius: 10, border: "1px solid #bbf7d0" }}>
                          ...
                          {soldEntries.map(([cid, qty]) => {
                            ...
                            const rev = eventCookieRevenue(event, Number(cid));
                            return (
                              <div key={cid} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "4px 0", borderBottom: "1px solid #d1fae5" }}>
                                <span style={{ fontSize: 13, fontWeight: 600 }}>{COOKIE_EMOJI[c?.name] || "🍪"} {c?.name}</span>
                                <div style={{ display: "flex", gap: 8 }}>
                                  <Badge color="green">{qty} boxes</Badge>
                                  <Badge color="purple">{fmt(rev)}</Badge>
                                </div>
                              </div>
                            );
                          })}
                            ...
                            {[...event.transactions].reverse().map((tx) => (
                              <div key={tx.txId} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "5px 8px", marginBottom: 4, background: "white", borderRadius: 8, border: "1px solid #e5e7eb", fontSize: 13 }}>
                                ...
                                <button onClick={(e) => { e.stopPropagation(); undoSale(event.id, tx.txId); }}
                                  style={{ padding: "4px 10px", borderRadius: 6, border: "1px solid #fca5a5", background: "#fef2f2", color: "#dc2626", fontFamily: fonts, fontWeight: 700, fontSize: 11, cursor: "pointer", whiteSpace: "nowrap" }}>
                                  ↩ Undo
                                </button>
                              </div>
                            ))}
          ...
          <div>
            ...
            <div style={{ background: "white", borderRadius: 14, padding: 14, marginBottom: 16, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", border: "1px solid #f3f4f6" }}>
              <textarea value={newNote} onChange={(e) => setNewNote(e.target.value)}
                placeholder="Add a note... (IOUs, discrepancies, shared inventory notes, reminders)"
                rows={3} style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1px solid #d1d5db", fontFamily: fonts, fontSize: 14, fontWeight: 600, resize: "vertical", boxSizing: "border-box" }} />
              <button onClick={addNote} disabled={!newNote.trim()}
                style={{ marginTop: 8, width: "100%", padding: 12, borderRadius: 10, border: "none", background: newNote.trim() ? "#047857" : "#d1d5db", color: "white", fontFamily: fonts, fontWeight: 800, fontSize: 14, cursor: newNote.trim() ? "pointer" : "not-allowed" }}>
                + Add Note
              </button>
            </div>
            ...
            {notes.length === 0 && (
              <div style={{ textAlign: "center", padding: 40, color: "#9ca3af", fontWeight: 600 }}>
                <div style={{ fontSize: 48, marginBottom: 8 }}>📝</div>No notes yet. Add IOUs, discrepancies, or reminders above.
              </div>
            )}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, color: "#9ca3af", fontWeight: 600, marginBottom: 4 }}>
                      📅 {new Date(note.timestamp).toLocaleString([], { weekday: "short", month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}
                    </div>
                    <div style={{ fontSize: 14, fontWeight: 600, whiteSpace: "pre-wrap", lineHeight: 1.5 }}>{note.text}</div>
                  </div>
                  <button onClick={() => deleteNote(note.id)}
                    style={{ padding: "4px 8px", borderRadius: 6, border: "1px solid #fca5a5", background: "#fef2f2", color: "#dc2626", fontFamily: fonts, fontWeight: 700, fontSize: 11, cursor: "pointer", marginLeft: 8, whiteSpace: "nowrap" }}>
                    🗑️
                  </button>
                </div>
              </div>
            ...
            {history.map((h) => (
              <div key={h.id} style={{ background: "white", borderRadius: 12, padding: "12px 14px", marginBottom: 8, boxShadow: "0 1px 3px rgba(0,0,0,0.04)", display: "flex", alignItems: "center", gap: 12, border: "1px solid #f3f4f6" }}>
                ...
                <Badge color={h.type === "sale" ? "amber" : h.type === "restock" || h.type === "undo" ? "green" : "red"}>
                  {h.type === "sale" ? `-${h.qty}` : h.type === "restock" || h.type === "undo" ? `+${h.qty}` : `-${h.qty}`}
                </Badge>
              </div>
            ))}
          </div>
